...
![The same window, three ways it goes bad: a wrong fact, a missing piece, or noise crowding the signal.](images/three-failures.png)
...
![A brake for each failure: Complete fights eagerness, Correct fights misalignment, Concise fights context-rot.](images/cheatsheet.png)
...
![Errors high in the stack multiply: one wrong line of code costs a line; one wrong line in the plan costs a hundred; one wrong assumption in research costs thousands.](images/cost-asymmetry.png)
...
![The shape of a compaction: goal, current state, decisions with why, what was ruled out, load-bearing assumptions, open threads, and pointers.](images/compaction-shape.png)
...
![Scale across two axes: across time, one clean context seeds the next; across people, the same context becomes a shared team substrate.](images/scale-time-people.png)
